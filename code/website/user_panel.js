...
audio.addEventListener('timeupdate', () => {
  if (audio.duration) {
    ...
    // Next Song Preload
    if (audio.currentTime > (audio.duration - 40) && PreDonwloadOn == false && PreNextSong.length == 0) {
        PreDonwloadOn = true
        PreDownloadNext()
    }
  }
...
let PreDonwloadOn = false

async function PreDownloadNext() {
    let randomSong = randomSelect()
    if (randomSong[0] in reuseDict) {
        PreNextSong = [randomSong[0], randomSong[1]]
        PreDonwloadOn = false
    } else {
        let req = await fetch(`/music_api`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({type: 'get', song_name: randomSong[0]}),
        });
        if (req.status == 200) {
            let blob = await req.blob();
            let url_blob = URL.createObjectURL(blob);
            reuseDict[randomSong[0]] = url_blob
            PreNextSong = [randomSong[0], randomSong[1]]
            PreDonwloadOn = false
        }
    }
...
function randomSelect() {
    // Returns an url (decoded) and the img url (decoded)
    songList = document.getElementsByClassName("music-card-text");
    i = Math.floor(Math.random() * songList.length);
    return [decodeHtml(songList[i].innerHTML), decodeHtml(songList[i].parentElement.querySelector('.music-ico').src)]
}

let PreNextSong = []

function nextSong() {
    if (PreNextSong.length == 0) {
        let randomSong = randomSelect()
        playSong(randomSong[0], randomSong[1])
    } else {
        playSong(PreNextSong[0], PreNextSong[1])
        PreNextSong = []
    }
}
